...
import { Camera, Droplet, Gauge, Power, Sparkles, Upload, ArrowDownToLine, Loader2, Activity, AlertTriangle } from "lucide-react";
    ...
    <div className="min-h-screen bg-[#0A0A0A]">
      ...
      <main className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
          <div>
            <div className="overline mb-1.5">// live console</div>
            <h1 className="font-display text-3xl sm:text-4xl font-black tracking-tight">Dashboard</h1>
            <p className="text-neutral-500 text-sm mt-1">
              Real-time telemetry · {activeSession ? <span className="text-cyan-400">SESSION RUNNING</span> : <span>Standby</span>}
            </p>
          </div>
          <div className="flex gap-2">
            {!activeSession ? (
              <Button onClick={startSession} data-testid="start-session-btn"
                className="bg-cyan-500 hover:bg-cyan-400 text-[#0A0A0A] font-bold h-11 px-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <SensorGauge value={analysis?.cleanliness_score ?? 0} min={0} max={100} unit="% CLEAN"
            ...
            testId="gauge-cleanliness" />
        </div>

        {/* Camera + Controls */}
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          {/* Camera */}
          <div className="lg:col-span-2 card-flat p-4 sm:p-5" data-testid="camera-card">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <Button size="sm" onClick={() => fileRef.current?.click()} disabled={analyzing}
                  ...
                  {analyzing ? <Loader2 size={14} className="animate-spin mr-1.5" /> : <Sparkles size={14} className="mr-1.5" />}
                  ANALISA AI
                </Button>
              </div>
            </div>
            <div className="aspect-video relative rounded-md overflow-hidden border border-[#262626] bg-[#0A0A0A]">
              <img
              ...
              <div className="absolute top-3 left-3 px-2 py-1 rounded bg-black/70 backdrop-blur text-[10px] font-mono uppercase tracking-widest text-cyan-400 border border-cyan-500/30">
                ● LIVE · CAM01
              </div>
              <div className="absolute bottom-3 right-3 px-2 py-1 rounded bg-black/70 backdrop-blur text-[10px] font-mono text-neutral-300">
                {new Date().toLocaleTimeString("id-ID")}
              </div>
            </div>
            {analysis && (
              <div className="mt-3 p-3 rounded-md bg-[#0E1A1F] border border-cyan-500/20" data-testid="ai-analysis-result">
                <div className="flex items-start gap-2">
                  <Sparkles size={14} className="text-cyan-400 mt-0.5 shrink-0" />
                  <div className="flex-1">
                    <div className="text-sm text-neutral-200">{analysis.description}</div>
          ...
          {/* Manual control */}
          <div className="card-flat p-4 sm:p-5 space-y-5" data-testid="control-panel">
            <div className="flex items-center justify-between">
              <div className="overline">Manual Control</div>
              <div className="flex items-center gap-2">
                <span className="text-[11px] text-neutral-500 uppercase tracking-widest">Auto</span>
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="font-mono tabular text-cyan-400 text-sm">{control?.motor_speed ?? 0}%</span>
              </div>
            ...
            {/* Nozzle */}
            <div className="flex items-center justify-between p-3 rounded-md bg-[#0E0E0E] border border-[#262626]">
              <div className="flex items-center gap-2">
                <Droplet size={14} className="text-cyan-400" />
                <span className="text-sm font-medium text-neutral-200">Water Nozzle</span>
              </div>
            ...
            <Button onClick={pushServo}
              ...
              className="w-full h-11 bg-[#1E1E1E] hover:bg-[#262626] text-neutral-100 border border-[#262626] font-medium">
              <ArrowDownToLine size={15} className="mr-2 text-cyan-400" />
            ...
            <Dialog open={thrOpen} onOpenChange={setThrOpen}>
              <DialogTrigger asChild>
                <Button variant="ghost" size="sm" data-testid="threshold-settings-btn"
                  className="w-full text-xs text-neutral-500 hover:text-cyan-400">
                  <AlertTriangle size={12} className="mr-1.5" />THRESHOLD AUTO-MODE
                </Button>
              </DialogTrigger>
              <DialogContent className="bg-[#121212] border-[#262626]">
                <DialogHeader>
                ...
                <div className="space-y-4 py-2">
                  <div>
                    ...
                    <Input type="number" defaultValue={control?.threshold_turbidity ?? 50}
                      ...
                      className="mt-2 bg-[#0A0A0A] border-[#262626] font-mono" />
                  </div>
                ...
                <DialogFooter>
                  <Button onClick={() => setThrOpen(false)} className="bg-cyan-500 hover:bg-cyan-400 text-[#0A0A0A]">
                    Tersimpan
                  </Button>
                </DialogFooter>
              </DialogContent>
